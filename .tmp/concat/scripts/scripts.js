...
app.controller('loginWindowCtrl', ["$scope", "$modalInstance", "data", "md5", "$http", "$timeout", "$cookieStore", function($scope, $modalInstance, data, md5, $http, $timeout, $cookieStore) {
    ...
    $scope.login = function(){
        $http({method: 'POST', url: 'ajax/index.php/login',data:{'name':$scope.userName,'pass':angular.isDefined($scope.userPass)?md5.createHash($scope.userPass):''}})
...
}).directive('text', ["$compile", function($compile) {
    return {
        restrict: 'A',
        transclude: true,
        replace: true,
...
angular.module('toolApp').
	...
	this.generateCode = function(type,data){
		$http({method: 'POST', url: 'ajax/index.php/make',data:{'type':type,'jsondata':data}})
                .success(function(data) {
    ...
    this.saveData = function(data, name){
        $http({method: 'POST', url: 'ajax/index.php/curd/save',data:{'jsondata':data,'username':name}})
                .success(function(data) {
                   $rootScope.$broadcast('savestatus', data); 
                });
    };
    this.updateData = function(data, jid){
        $http({method: 'POST', url: 'ajax/index.php/curd/update',data:{'jsondata':data,'jid':jid}})
                .success(function(data) {
                   $rootScope.$broadcast('updatestatus', data); 
                });
    };
    this.getJsonList = function(method, name, page, jid, status){
        $http({method: 'POST', url: 'ajax/index.php/curd/'+method,data:{'username':name,'page':page,'jid':jid,'status':status}})
                .success(function(data) {
                   $rootScope.$broadcast('getlist', data);
        });
    };
    this.open = function(jid){
        $http({method: 'POST', url: 'ajax/index.php/curd/open',data:{'jid':jid}})
                .success(function(data) {
                   $rootScope.$broadcast('open', data); 
                });
    };
    this.getDate = function(method,name,bpass,pass){
        $http({method: 'POST', url: 'ajax/index.php/curd/'+method,data:{'username':name,'beforepass':bpass,'pass':pass}})
                .success(function(data) {
                   $rootScope.$broadcast('userDataReady', data); 
                });
    };
    this.preViewt = function(json,name,type){
        $http({method: 'POST', url: 'ajax/index.php/make/preview',data:{'data':json,'name':name,'type':type}})
                .success(function(data) {
